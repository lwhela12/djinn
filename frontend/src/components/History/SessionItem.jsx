  ...
  return (
    <div className="glass-panel p-4 flex justify-between items-center">
      <Link to={`/sessions/${session.id}`} className="flex-1 cursor-pointer">
        <div className="font-accent text-lg text-ethereal-gold">
          {session.title || 'Untitled Session'}
        </div>
        <div className="text-sm text-sage-mist">{session.status}</div>
      </Link>
      <button
        onClick={() => onDelete(session.id)}
        className="ml-2 p-1 rounded-full hover:bg-error-crimson/20 focus:outline-none focus:ring-2 focus:ring-error-crimson"
        aria-label="Delete session"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-error-crimson" viewBox="0 0 20 20" fill="currentColor">
